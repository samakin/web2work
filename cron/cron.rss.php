...
$short_story = htmlspecialchars ($short_story, ENT_QUOTES, $config['charset']);
$full_story = htmlspecialchars ($full_story, ENT_QUOTES, $config['charset']);
...
$short_story = rss_strip ($short_story);
$full_story = rss_strip ($full_story);
$city_name = $channel_info['add_city'];
								$cat_info_firm_citys = get_vars ( "citys_onportal_v1" );
								foreach ($cat_info_firm_citys as $eac)
								{
									if ($eac[name]==$city_name and $city_name!='' and $eac[name]!='') $city = $eac[id];
								}
...
$result = $db->query( "UPDATE ".PREFIX ."_post set city='$city', date='$str_date', title='$title', short_story='$short_story', full_story='$full_story', descr='$descr', keywords='$keywords', category='$category', alt_name='$alt_name', allow_comm='$allow_comm', approve='$approve', allow_main='$allow_main',  xfields='$xfields',  descr='$descr',  keywords='$key_words' $tes $fgs WHERE id='$news_id'");
@unlink( ENGINE_DIR . '/cache/system/all_news_post_'.$category.'.php' );
...
$db->query ( "INSERT INTO ".PREFIX ."_post (autor, city, category, date, title, alt_name, short_story, full_story, xfields, allow_main, approve, allow_comm, allow_br, fixed, descr, keywords $tes $fgs) VALUES ('$author', '$city', '$category', '$str_date', '$title', '$alt_name', '$short_story', '$full_story', '$xfields', '$allow_main', '$approve', '$allow_comm', '1', '0', '$descr', '$key_words' $te $fg)");
@unlink( ENGINE_DIR . '/cache/system/all_news_post_'.$category.'.php' );
$news_id = $db->insert_id();
$db->query ("INSERT INTO " . PREFIX . "_post_extras (news_id, allow_rate, user_id) VALUES('".$news_id."', '1', '')");
...
@unlink( ENGINE_DIR . '/cache/system/all_news_top.php' );

return true;
};
include (ENGINE_DIR . '/modules/content/all_news/create_array.php');
?>